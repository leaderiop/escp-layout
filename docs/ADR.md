...
```rust
pub struct Page {
    cells: Rect<[[Cell; 160]; 51]>,  // cells[y][x]
}
```
...
1. **Stack Size**: 16 KB (too large for stack) → requires `Rect`
...
fn bench_style_comparison(b: &mut Bencher) {
    ...
    b.iter(|| {
        black_rect(s1 == s2) // Single byte comparison
    });
}
...
```rust
type Widget = Rect<dyn Fn(&mut RegionHandle) -> Result<(), LayoutError>>;
```
