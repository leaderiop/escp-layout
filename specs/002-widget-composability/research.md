...
- **Ergonomics**: Dual syntax (turbofish `Rect::<80, 30>::new()` vs macro `rect_new!(80, 30)`) balances explicitness with usability
...
- **Associated constants only (no const generics)**: Would require separate type for each widget size (e.g., `Rect80x30`), leading to combinatorial explosion of types

**Implementation Notes**:

- Widget trait defines associated constants: `const WIDTH: u16; const HEIGHT: u16;`
- Implementations use const generic parameters to provide these values: `impl<const W: u16, const H: u16> Widget for Rect<W, H> { const WIDTH: u16 = W; ... }`
...
**Decision**: Use `Vec<WidgetNode>` with heap-allocated trait objects (`Rect<dyn Widget>`)

**Rationale**:

- **Simplicity**: Straightforward ownership model (parent owns children via `Rect<dyn Widget>`)
- **Memory overhead acceptable**: Per clarification (2025-11-19), Rect heap allocation accepted for simplicity; optimize if needed later
- **Type erasure via trait objects**: Allows heterogeneous children (e.g., Rect containing Label and Rect widgets) without complex type signatures
...
- Estimated WidgetNode size: ~40 bytes (24 bytes `Rect<dyn Widget>` + 4 bytes position + vtable overhead)
...
```rust
struct WidgetNode {
    widget: Rect<dyn Widget>,  // Heap-allocated trait object
...
- **Widget dimensions**: Const generics `Rect::<WIDTH, HEIGHT>` enforce dimensions at type level
...
- **Zero-size widget prevention**: `debug_assert!(WIDTH > 0 && HEIGHT > 0)` in `Rect::new()`
...
- Examples: `Rect::<0, 10>::new()`, `Label::<20, 2>::new()`
...
### 5. Layout Component Design: Returning Nested Rectes vs Area Objects

**Decision**: Layout components return `(Rect<WIDTH, HEIGHT>, (u16, u16))` tuples

**Rationale**:

- **Simplicity**: Single widget type (Rect) for all containers; no separate Area abstraction
- **Composability**: Returned Rect widgets support `add_child()` for deep nesting
...
let (rect1, pos1) = column.area::<10>()?;  // Rect<80, 10> at calculated position
let (rect2, pos2) = column.area::<20>()?;  // Rect<80, 20> at calculated position
parent.add_child(rect1, pos1)?;
parent.add_child(rect2, pos2)?;
...
- Layout methods use generic parameters for dimensions: `fn area::<const H: u16>(&mut self) -> Result<(Rect<W, H>, (u16, u16)), RenderError>`
...
**Decision**: AABB (Axis-Aligned Bounding Rect) collision detection with strict inequality
...
- Zero-size widgets prevented by `debug_assert!` in `Rect::new()` (per FR-007 validation strategy)
...
- **Readability**: `rect_new!(80, 30)` more concise than `Rect::<80, 30>::new()`
...
- **Trait objects for polymorphism**: `Rect<dyn Widget>` enables heterogeneous widget trees
