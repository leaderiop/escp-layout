...
_Re-check after Phase 1 design (research.md, data-model.md, contracts/ completed)_
...
- contracts/ define stable rendering order in DynamicRect::render_to()
...
- Widget trait uses runtime `width()` and `height()` methods (not const generics due to DynamicRect requirement)
- Layout components return DynamicRect with runtime dimensions (compatible with V1 static layout via explicit size parameters)
...
- dynamic_rect.rs specifies Result-based error handling (composition)
...
- Rect<dyn Widget> uses Rust std allocation
...
- DynamicRect created with explicit dimensions (no auto-sizing)
...
- dynamic_rect.rs specifies checked arithmetic for overflow detection
...
1. Widget trait uses runtime `width()`/`height()` methods instead of const generics (enables DynamicRect for layouts)
2. Layout components return DynamicRect with runtime dimensions (compatible with fixed-layout via explicit parameters)
