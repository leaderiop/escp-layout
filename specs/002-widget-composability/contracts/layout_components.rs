...
/// Divides parent Box space vertically, returning nested DynamicBox widgets
/// for each row.
///
...
impl Column {
    ...
    ///
    /// # Parameters
    /// - `width`: Parent Box width (all rows will have this width)
    /// - `height`: Parent Box height (total vertical space to divide)
    ///
    /// # Example
    /// ```rust,ignore
    ...
    ///
    /// Returns a DynamicBox with dimensions (width, height) and its relative
    /// position within the parent Box. Position is automatically calculated
    ...
    ///
    /// # Returns
    /// - `(DynamicBox, (u16, u16))`: Box widget and its (x, y) position
    ...
    /// let mut column = Column::new(80, 30);
    ///
    /// let (mut row1, pos1) = column.area(10)?; // Returns Box<80, 10> at (0, 0)
    /// row1.add_child(Label::new("Row 1"), (0, 0))?;
    /// root.add_child(row1, pos1)?;
    ///
    /// let (mut row2, pos2) = column.area(10)?; // Returns Box<80, 10> at (0, 10)
    /// row2.add_child(Label::new("Row 2"), (0, 0))?;
    /// root.add_child(row2, pos2)?;
    /// ```
    pub fn area(&mut self, height: u16) -> Result<(DynamicBox, (u16, u16)), RenderError>;
}
...
/// Divides parent Box space horizontally, returning nested DynamicBox widgets
/// for each column.
///
...
pub struct Row {
    ...
    /// Parent height (all returned Boxes have this height)
    height: u16,
...
impl Row {
    ...
    ///
    /// # Parameters
    /// - `width`: Parent Box width (total horizontal space to divide)
    /// - `height`: Parent Box height (all columns will have this height)
    pub fn new(width: u16, height: u16) -> Self;

    /// Allocate a vertical area (column) with specified width.
    ///
    /// Returns a DynamicBox with dimensions (width, height) and its relative
    /// position within the parent Box.
    ///
    /// # Parameters
    ...
    ///
    /// let (mut col1, pos1) = row.area(25)?; // Returns Box<25, 30> at (0, 0)
    /// col1.add_child(Label::new("Column 1"), (0, 0))?;
    /// root.add_child(col1, pos1)?;
    ///
    /// let (mut col2, pos2) = row.area(25)?; // Returns Box<25, 30> at (25, 0)
    /// col2.add_child(Label::new("Column 2"), (0, 0))?;
    /// root.add_child(col2, pos2)?;
    /// ```
    pub fn area(&mut self, width: u16) -> Result<(DynamicBox, (u16, u16)), RenderError>;
}
...
/// Returns overlapping nested DynamicBox widgets at the same position.
///
/// Stack layout is used for layering widgets (e.g., background + foreground).
/// All calls to `area()` return Boxes with the same dimensions at position (0, 0).
///
/// # Rendering Order
/// Later children obscure earlier children when overlapping (depth-first
/// rendering order in DynamicBox).
pub struct Stack {
    /// Parent width (all returned Boxes have this width)
    width: u16,
...
impl Stack {
    ...
    ///
    /// # Parameters
    /// - `width`: Parent Box width (all layers will have this width)
    /// - `height`: Parent Box height (all layers will have this height)
    pub fn new(width: u16, height: u16) -> Self;

    /// Allocate an overlapping area (same position for all calls).
    ///
    /// Returns a DynamicBox with full parent dimensions at position (0, 0).
    ...
    ///   (propagated from DynamicBox::new)
    ///
    /// # Example
    /// ```rust,ignore
    /// let mut root = DynamicBox::new(80, 30)?;
    ...
    ///
    /// // Background layer
    /// let (mut bg, pos) = stack.area()?; // Returns Box<80, 30> at (0, 0)
    /// bg.add_child(Background::new(), (0, 0))?;
    /// root.add_child(bg, pos)?;
    ///
    /// // Foreground layer (overlays background)
    /// let (mut fg, pos) = stack.area()?; // Returns Box<80, 30> at (0, 0)
    /// fg.add_child(Label::new("Overlay"), (10, 10))?;
    /// root.add_child(fg, pos)?;
    /// ```
    pub fn area(&self) -> Result<(DynamicBox, (u16, u16)), RenderError>;
}
