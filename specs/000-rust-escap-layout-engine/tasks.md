...
- [x] T016 [P] [US1] Implement Page struct in src/page.rs
  ...
  - `struct Page { cells: Rect<[[Cell; 160]; 51]> }`
...
- [x] T017 [P] [US1] Implement PageBuilder struct in src/page.rs
  ...
  - `struct PageBuilder { cells: Rect<[[Cell; 160]; 51]> }`
...
**Goal**: Implement 6 widget types (Label, TextBlock, Paragraph, ASCIIRect, KeyValueList, Table)
...
- ASCIIRect draws borders correctly
...
- [x] T065 [P] [US6] Implement ASCIIRect widget in src/widgets/ascii_rect.rs

  - `pub struct ASCIIRect { title: Option<String>, content: Rect<dyn Widget> }`
  - `ASCIIRect::new(content: Rect<dyn Widget>) -> ASCIIRect`
  - `ASCIIRect::with_title(title) -> ASCIIRect`
  - Implement Widget trait
  - Draw border using +, -, | characters
  - Render title in top border if present
  - Render content in inset region (1-cell padding)
  - Handle regions too small for rect (< 3×3)

- [x] T066 [US6] Add unit tests for ASCIIRect in tests/unit/widget_tests.rs
  - Test rect without title
  - Test rect with title
...
- [x] T076 [P] Add rustdoc comments to all widgets in src/widgets/
  ...
  - Widget trait
  - Label, TextBlock, Paragraph, ASCIIRect, KeyValueList, Table
